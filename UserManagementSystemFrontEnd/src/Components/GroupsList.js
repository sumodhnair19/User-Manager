        ...
        return  groups.map((group) => {
            return (
                <tr key={group._id}>
                    ...
                    <td className="xs-visible">{moment(group.created).format('DD.MMM.YY - hh:mm')}</td>
                    <td>
                        {this.state.updatedid === group._id ? (
                        <button className="btn" type="button" onClick={()=> this.handleUpdateBox("", "")}><i className="fa fa-times"></i></button>
                        ):(
                        <button className="btn" type="button" onClick={()=> this.handleUpdateBox(group._id, group.title)}><i className="fa fa-edit"></i></button>
                        )}
                        <button
                        className="btn "
                        type="button"
    ...
    render(){
        ...
        return(
            <table className="table table-responsive-sm">
                ...
                <tbody>
                {groups && groups.length>0 && this.renderGroups(groups)}
                </tbody>
            </table>
        )
    }
